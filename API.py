...
def genMask():
    ...
    while True:
        ...
        colorLow = np.array([140, 61, 61])
        colorHigh = np.array([215, 185, 255])
...
def genIdOverlay():
    cap = cv2.VideoCapture(cam_uri+"1004/")
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 854)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
    font = cv2.FONT_HERSHEY_DUPLEX
    i=0
    mc = memcache.Client(['127.0.0.1:11211'], debug=0)
    dict=dict = mc.get("positions")
    ...
    while True:
        ...
        if(dict != None):
            for id in dict:
                cv2.putText(frame, str(id), (int(dict[id][0]*(854/500)), int(dict[id][1]*(450/500))), font, 1,(255,255,255),2,cv2.LINE_AA)
        im_enc = cv2.imencode('.jpeg', frame, [cv2.IMWRITE_JPEG_QUALITY, 200])[1]
        i+=1
        yield (b'--frame\r\n'
               b'Content-Type: image/jpeg\r\n\r\n' + im_enc.tobytes() + b'\r\n')
